package com.vela.im.service.conversation.infrastructure.persistence.mapper;
...
public interface ImConversationSetMapper extends BaseMapper<ImConversationSetEntity> {
    ...
    @Update(" update vela_conversation_set set readed_sequence = #{readedSequence},sequence = #{sequence} " +
    ...
    @Select(" select max(sequence) from vela_conversation_set where app_id = #{appId} AND from_id = #{userId} ")
